...
Loop::Loop()
    ...
    // Window
    windowTitle = "Gpu Fluid Simulator - Laura Wang Qiu";
    windowWidth = 640;
    windowHeight = 480;
...
void Loop::update() {
    for (auto& p : particles) {
        if (p.timeLeft != PARTICLES_MAX_TIME_LEFT) {
    ...
    particles.erase(
        std::remove_if(particles.begin(), particles.end(),
                       [this](const Particle& p) {
                           return (p.timeLeft <= 0 && p.timeLeft != PARTICLES_MAX_TIME_LEFT) ||
                               (p.posX + p.radius < 0 &&
                                p.posX - p.radius < 0) ||
                               (p.posX - p.radius > windowWidth &&
                                p.posX + p.radius > windowWidth) ||
                               (p.posY + p.radius < 0 && p.posY - p.radius < 0) ||
                               (p.posY - p.radius > windowHeight &&
                                p.posY + p.radius > windowHeight);
                       }),
        particles.end());
}
...
void Loop::renderInterface() {
    ...
    if (ImGui::CollapsingHeader("Particle Settings")) {
        ...
        ImGui::SliderFloat(" ", &particleTimeLeft, PARTICLES_MIN_TIME_LEFT, PARTICLES_MAX_TIME_LEFT, "%.1f seconds");
        if (particleTimeLeft == PARTICLES_MAX_TIME_LEFT) {
            ImGui::SameLine();
            ImGui::Text("inf");
        }
    }
...
void Loop::generateParticleSea() {
    ...
    for (int y = 0; y < rows; ++y) {
        for (int x = 0; x < columns; ++x) {
            ...
            p.density = particleDensity;
            p.color = particleCol;
            p.timeLeft = PARTICLES_MAX_TIME_LEFT;
